...
(function ($) {
    ...
    BootstrapTable.prototype.initSearch = function () {
        ...
        if (this.options.sidePagination !== 'server') {
            ...
            var sss = this.searchText && (this.options.escape ?
...
!function ($) {
    ...
    BootstrapTable.prototype.initSearch = function () {
        ...
        if (this.options.sidePagination !== 'server') {
            // Alteração para encontrar duas ou mais palavras separada por espaço em branco
            var original = this.searchText && this.searchText.toLowerCase();
            var todas = original ? original.split(",") : [];
            var local_data = [];

            $.each(todas, function(index, conteudo) {
                var s = conteudo.trim();
                var s_data = s ? $.grep(that.options.data, function (item) {
                    for (var key in item) {
                ...
                local_data = local_data.concat(s_data);
            });
            local_data = local_data.filter(function(item, pos) {
                return local_data.indexOf(item) == pos;
            })
            if (local_data.length > 0) {
                that.data = local_data;
            } else {
    ...
    BootstrapTable.prototype.resetHeader = function () {
        ...
        setTimeout(function () {
            $fixedHeader.css({
                ...
                .append(that.$header_).removeClass('table');
